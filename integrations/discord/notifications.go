...
// Kept as a template for a Discord-specific message if one is ever needed.
func NotifyNewAdmin(ctx context.Context, pool *pgxpool.Pool, discordUserID, adminURL, tempPassword string) {
	var text string
	if adminURL != "" {
	...
	if err := PostDirectMessage(ctx, discordUserID, text); err != nil {
		log.Printf("discord: NotifyNewAdmin to %s: %v", discordUserID, err)
	}
}
...
//
// Scaffold — not yet wired: the Discord report flow creates the work item but
// does not notify admins. Slack's equivalent is called from its report flow
...
func NotifyAdminsNewWorkItem(ctx context.Context, pool *pgxpool.Pool, item store.WorkItem) {
	...
	if err != nil {
		log.Printf("discord: NotifyAdminsNewWorkItem: list admins: %v", err)
		return
	}
	...
	for _, admin := range admins {
		if err := n.NotifyUser(ctx, admin.ID, platform.Message{Text: text}); err != nil {
			log.Printf("discord: NotifyAdminsNewWorkItem: notify %s: %v", admin.ID, err)
		}
	}
}
