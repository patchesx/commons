...
// HandleUploadToVimeo handles POST /api/vimeo/upload.
// Accepts a multipart form with: file (video), title, category, description (optional).
// Uploads to Vimeo as unlisted, then creates a resource library entry.
func HandleUploadToVimeo(pool *pgxpool.Pool, encKey []byte) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		videoID, _, err := UploadVideo(ctx, pool, encKey, file, fh.Size, title, description, "unlisted")
		if err != nil {
		...
		if err := store.CreateResource(ctx, pool, resource); err != nil {
			log.Printf("vimeo/upload: created video %s but failed to create resource entry: %v", videoID, err)
		}

		store.LogAction(ctx, pool, nil, "vimeo.uploaded", resource.ID, map[string]string{
			"title":    title,
			"video_id": videoID,
		})
