...
// UploadData holds Vimeo-specific upload metadata for a recording_upload job.
// Stored in vimeo.upload_data.
...
// CreateUploadData inserts a new vimeo.upload_data row for a job.
func CreateUploadData(ctx context.Context, pool *pgxpool.Pool, d *UploadData) error {
	_, err := pool.Exec(ctx, `
		INSERT INTO vimeo.upload_data (job_id, integration_id, title, meeting_topic, meeting_date, duration_secs)
		VALUES ($1, $2, $3, $4, $5, $6)
...
// GetUploadData returns the vimeo.upload_data row for a job. Returns nil, nil if not found.
func GetUploadData(ctx context.Context, pool *pgxpool.Pool, jobID string) (*UploadData, error) {
	...
	err := pool.QueryRow(ctx, `
		...
		FROM vimeo.upload_data WHERE job_id = $1
...
// SetVideoID updates the video_id on a vimeo.upload_data row.
func SetVideoID(ctx context.Context, pool *pgxpool.Pool, jobID, videoID string) error {
	_, err := pool.Exec(ctx,
		`UPDATE vimeo.upload_data SET video_id = $1 WHERE job_id = $2`,
		videoID, jobID,
	)
	return err
}
