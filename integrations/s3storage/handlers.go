...
// HandleStorageCard returns the S3 storage locations card fragment.
// GET /admin/fragments/settings/s3-storage
func HandleStorageCard(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()

		integ, err := store.GetIntegrationByType(ctx, pool, "s3")
		if err != nil || integ == nil || !integ.Enabled {
			w.WriteHeader(http.StatusNoContent)
			return
		}

		locations, _ := store.ListStorageLocationsByType(ctx, pool, "s3")
...
func renderStorageCard(integrationID string, locations []store.StorageLocation) string {
	...
	var locRows string
	if len(locations) == 0 {
		locRows = `<p class="text-sm text-gray-500 dark:text-gray-400 mb-3">No storage locations added. Add an S3 key prefix to set where recordings are uploaded.</p>`
	} else {
...
<div class="mt-3 border-t border-gray-100 dark:border-gray-700 pt-3">
  ...
  <div x-show="adding" style="display:none" class="space-y-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
      ...
      <div>
        <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1">Key Prefix</label>
        <input
          type="text"
          x-model="path"
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow">
  <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
    <h2 class="text-base font-semibold text-gray-800 dark:text-white">S3 Storage — Key Prefixes</h2>
  </div>
